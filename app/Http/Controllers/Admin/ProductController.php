...
class ProductController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        // Get trace_code in Product
        $trace_code = null;
        $productHistories = null;
        $product = app($dataType->model_name)::with('productHistory')->where('id', $id)->first();
        if ($product && $product->trace_code) {
            $trace_code = $product->trace_code;
        }
        if ($product && $product->productHistory) {
            $productHistories = $product->productHistory;
        }
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'trace_code', 'productHistories'));
    }

    public function addStages(Request $request, $id)
    {
        $slug = 'products';
        ...
        $view = 'voyager::bread.add-stage';

        if (view()->exists("voyager::$slug.add-stage")) {
            $view = "voyager::$slug.add-stage";
        }

        $latestStage = $productHistories->last();

        $stages = Stage::get();
        if ($latestStage) {
            $stages = Stage::where('id', '>=', $latestStage->stage_id)->get();
        }

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'trace_code', 'productHistories', 'stages'));
    }

    public function postAddStages(Request $request, $id)
    {
        $slug = 'product-histories';
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        // Check permission
        ...
        $productHistory->stage_id = $request->get('stage_id');
        $productHistory->trace_code = $request->get('trace_code');
        $productHistory->tx_hash = $request->get('tx_hash');
        $productHistory->data = $request->get('stage_data');
        ...
        if ($request->hasFile('images')) {
            ...
            foreach ($images as $image) {
                $path = $image->store('product_histories', 'public');
                $tempImages[] = $path;
            }
        }
        $productHistory->images = json_encode($tempImages);
        $productHistory->save();
    ...
    public function edit(Request $request, $id)
    {
        ...
        $view = 'voyager::bread.edit';

        if (view()->exists("voyager::$slug.edit")) {
