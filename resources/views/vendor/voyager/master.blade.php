...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('resources/css/custom.css') }}?time={{ time() }}">
    @yield('css')
...
<body class="voyager @if (isset($dataType) && isset($dataType->slug)) {{ $dataType->slug }} @endif">
    ...
    <script>
        const historyRoute =
            "{{ $trace_code ?? false ? route('api.product-histories', ['trace_code' => $trace_code]) : '' }}";
        const assetUrl = "{{ asset('') }}storage/";
    </script>
    <script src="https://cdn.jsdelivr.net/npm/ethers@6.6.2/dist/ethers.umd.min.js"></script>
    <script type="module" src="{{ asset('resources/js/blockchain.js') }}?time={{ time() }}"></script>
    <script type="text/javascript" src="{{ asset('resources/js/custom.js') }}?time={{ time() }}"></script>
